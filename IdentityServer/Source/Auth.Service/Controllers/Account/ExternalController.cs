...
namespace Auth.Service.Controllers.Account
{
    ...
    public class ExternalController : Controller
    {
        ...
        public async Task<IActionResult> Callback()
        {
            ...
            (ApplicationUser user, string provider, string providerUserId, IEnumerable<Claim> claims) = await FindUserFromExternalProviderAsync(result);
            if (user is null)
            {
        ...
        private async Task<IActionResult> ProcessWindowsLoginAsync(string returnUrl)
        {
            ...
            if (result?.Principal is WindowsPrincipal wp)
            {
                ...
                ClaimsIdentity id = new(AccountOptions.WindowsAuthenticationSchemeName);
